...
- 자동 설정 점검 활성/비활성
- 모의투자/실거래 모드 선택
- 예산, 진입금, 손절, 익절, 트레일링 스탑, 스캔 종목 수 조정
- 설정 자동 점검
- AI 설정 분석
- rule 기반 추천값 적용
- 자금 현황 저장/조회
...
대시보드에서 바꾼 운용 설정은 `data/runtime_settings.json`에 저장됩니다. API 키 같은 민감 정보는 UI에 표시하거나 저장하지 않고 `.env`에서만 읽습니다.

설정 점검 모드는 현재 예산, 1회 진입 비중, 일일 손실 한도, 수수료/슬리피지 반영 기대수익, 손익비, 반복 주기, BTC 급락 차단 기준을 검사합니다. 자동 설정 점검을 켜면 지속 실행 로그에도 설정 안전 점수와 경고가 같이 기록됩니다. AI 설정 분석은 이 rule 기반 결과를 바탕으로 한국어 리스크 리뷰를 생성합니다.

참고: Cursor의 `crsr_` API 키는 일반 OpenAI 호환 `/v1/chat/completions` 채팅 엔드포인트가 아닐 수 있습니다. 이 경우 앱은 AI 호출 실패 메시지를 보여주고 rule 기반 설정 점검은 계속 제공합니다. 실제 LLM 요약을 쓰려면 OpenAI 호환 `AI_BASE_URL`과 해당 API 키를 `.env`에 설정하세요.
