"""실거래: Binance 계정 → 포트폴리오 동기화 (시뮬 데이터와 무관)."""
...
async def sync_live_portfolio(
  ...
  # AIDI 로컬 체결 로그 + 거래소 최근 체결 병합
    local_trades = [
        TradeEvent(**t) for t in live_meta.get("trades", [])[-100:]
    ]
    ...
    return f"[{net}] 거래소 연동 · 보유 {n}종 · 총자산 약 {total_krw:,.0f}원 (USDT {usdt_free:.2f})"
